...
const ManageFaculty = () => {
  ...
  const handleSaveFaculty = async () => {
    ...
    try {
      ...
      console.error(
        `Error ${editingFaculty ? "updating" : "adding"} faculty:`,
        err
      );
      alert(
        `Failed to ${editingFaculty ? "update" : "add"} faculty: ${err.message}`
      );
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setFilteredFaculties(
        filteredFaculties.filter((faculty) => faculty.ID !== id)
      );
    } catch (err) {
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {filteredFaculties.length} of {faculties.length} members
                </p>
              </div>
            </div>
